...
import { Image, TextInput, TouchableWithoutFeedback, TouchableOpacity, View , TextComponent,Text } from "react-native";
...
export default class PhoneInput extends Component {
  ...
  render() {
    const { arrowDropdownsize} =this.props;
    const { iso2, formattedNumber, disabled } = this.state;
    const TextComponent = this.props.textComponent || TextInput;
    return (
      <View style={[styles.container, this.props.style]}>
        <TouchableOpacity
          style={[!this.props.isVisibleTextInput?{flexDirection:'row',backgroundColor:'transparent',justifyContent:'center'}:null]}
        ...
        >
          ...
          {
            !this.props.isVisibleTextInput?
              <View style={{justifyContent:'center',flex:1.2, flexDirection:'row',justifyContent:'space-between'}}>
                ...
                size={arrowDropdownsize?arrowDropdownsize:24}
                color={"black"}
                />
